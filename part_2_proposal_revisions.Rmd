...
amertrends_subset <- amertrends_subset |> 
  mutate(econ_sit_w124 = fct_collapse(econ_sit_w124,
...
#re-coding newsuse_plat_a_w124 
amertrends_subset |> 
  drop_na(newsuse_plat_a_w124) |> 
  mutate(sm_pref_basicfacts = fct_collapse(newsuse_plat_a_w124,
                                      `prefers social media` = c("2"),
                                      `does not prefer social media` = c("1", "3", "4")))
#now need to change column type to logical and make "prefers social media" 1 and "does not prefer social media" 0. Could probably also do this without the previous recode actually.
amertrends_subset <- amertrends_subset |> 
  drop_na(newsuse_plat_a_w124) |> 
  mutate(sm_pref_basicfacts = fct_collapse(newsuse_plat_a_w124,
                                      `1` = c("2"),
                                      `0` = c("1", "3", "4"))) 
#visualize counts in each category  
ggplot(data = amertrends_subset) +
  geom_bar(aes(x = sm_pref_basicfacts)) +
  labs(x = "Social media preference: no (0) or yes (1)" )

  
  #mutate(sm_pref_index = sum(newsuse_plat_a_w124, 
                           newsuse_plat_b_w124, 
                           newsuse_plat_c_w124, 
                           newsuse_plat_d_w124, 
                           newsuse_plat_e_w124))
