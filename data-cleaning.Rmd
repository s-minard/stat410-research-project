...
```{r model_v1}
#model v1
autoc_lm_v1 <- glm(autoc_support ~ sm_pref_index + sm_freq + factor(econ_percep) + factor(age) + factor(race_1) + factor(pol_lean) + factor(income), family = binomial(link = "logit"), data = amertrends_subset)
summary(autoc_lm_v1)
car::vif(autoc_lm_v1)
...
```{r stepwise}
#creating "big" model 
big_v1<- glm(autoc_support ~ sm_pref_index + sm_freq + factor(econ_percep) + factor(age) + factor(race_1) + factor(pol_lean) + factor(income), family = binomial(link = "logit"), data = amertrends_subset)
#creating "small" model with only theory-driven variables that must be included at minimum, but no controls
small_v1<- glm(autoc_support ~ sm_pref_index + sm_freq + factor(econ_percep), family = binomial(link = "logit"), data = amertrends_subset)

#create stepwise model
step_v1 <- step(big_v1, scope = list(lower =  small_v1, upper = big_v1), direction = "both", trace = TRUE)

#model parameter estimates and significance testing
summary(step_v1)

# Drop NA values for all variables used in either model
clean_data <- amertrends_subset |> 
...
small_v1<- glm(autoc_support ~ sm_pref_index + sm_freq + factor(econ_percep), family = binomial(link = "logit"), data = clean_data)
big_v1<- glm(autoc_support ~ sm_pref_index + sm_freq + factor(econ_percep) + factor(age) + factor(race_1) + factor(pol_lean) + factor(income), family = binomial(link = "logit"), data = clean_data)
# Stepwise selection
step_model <- step(big_v1, scope = list(lower = small_v1, upper = big_v1), direction = "both")
```
