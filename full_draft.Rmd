...
amertrends_clean |> 
  ...
  geom_boxplot(aes(fill = autoc_support_cat),
               ...
               alpha = 0.75) +
  theme_light() +
  scale_fill_manual(values = c("brown1", "turquoise4")) +
...
amertrends_subset <- amertrends_subset|> 
  ...
  mutate(econ_sit_w124 = fct_recode(econ_sit_w124,
                          ...
                          "somewhat bad" = "3",
                          "very bad" = "4")) 
#frequency table for econ_sit
frq(amertrends_subset$econ_sit_w124, out = "v")
...
autoc_lm_v1 <- glm(autoc_support ~ sm_pref_index + sm_freq + factor(econ_percep) + factor(age) + factor(race_1) + factor(pol_lean)  + factor(pol_lean)*sm_pref_index + factor(income), family = binomial(link = "logit"), data = amertrends_clean)
summary(autoc_lm_v1)
#check VIF- collinearity issues between sm_pref_index*pol_lean interaction and pol_lean
car::vif(autoc_lm_v1)
...
small_v1<- glm(autoc_support ~ sm_pref_index + sm_freq + factor(econ_percep) + factor(pol_lean)*sm_pref_index , family = binomial(link = "logit"), data = amertrends_clean)
#big has optional demographic variables
big_v1<- glm(autoc_support ~ sm_pref_index + sm_freq + factor(econ_percep) + factor(age) + factor(race_1) + factor(pol_lean) + factor(pol_lean)*sm_pref_index + factor(income), family = binomial(link = "logit"), data = amertrends_clean)
...
summary(autoc_lm_v2)
#collinearity still an issue for the smaller model between the interaction sm_pref_index:pol_lean and pol_lean
car::vif(autoc_lm_v2)
...
plot(roc_v2, legacy.axes = TRUE, main = "ROC Curve for autoc_lm_v2", 
     ...
     ylab = "True Positive Rate (Sensitivity)")
auc(roc_v2)
#AUC gets slighter smaller but difference is negligible
...
#moving forward with model v2, checking assumptions
#1: large sample size - satisfied (n = 2065)

#2: independence of outcomes/variables:

#3: outliers

#4: model fits the data
```
...
```{r}
#| label: figure-drafts
#these can be moved into the appropriate places in the paper later
...
```{r}
#| echo: FALSE
frq(amertrends_subset$age, out = "txt")
```
...
```{r}
#| echo: FALSE
frq(amertrends_subset$income, out = "txt")
```
...
```{r}
#| echo: FALSE
frq(amertrends_subset$race_1, out = "txt")
```
...
```{r}
#| echo: FALSE
frq(amertrends_subset$econ_percep, out = "txt")
```
...
```{r}
#| echo: FALSE
frq(amertrends_subset$pol_lean, out = "txt")
```
